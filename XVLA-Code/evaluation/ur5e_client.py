"""
UR5e + X-VLA 仿真客户端（含 IK 控制 + 底座坐标变换）
"""
...
class UR5eMuJoCoEnv:
    ...
    def step(self, action_7d_world):
        ...
        self.data.ctrl[6] = 0.0 if grip > 0.5 else 255.0  # 夹爪控制
        for _ in range(5):
...
class XVLAClient:
    ...
    def parse_action(self, action_20d):
        ...
        rot6d_base = left[3:9]
        grip_logit = left[9]
        # rot6d → rotation matrix (base frame)
...
def run_evaluation(args):
    env = UR5eMuJoCoEnv(args.scene_xml)
    client = XVLAClient(host=args.server_ip, port=args.server_port, domain_id=args.domain_id, steps=10)
    ...
    for step in range(max_steps):
        ...
        pos_base, mat_base, grip_logit = client.parse_action(actions[0])
        ...
        if step % 10 == 0:
            print(f"  Step {step}/{max_steps} | "
                  f"pos_base={pos_base.round(3)} -> pos_world={target_pos_world.round(3)} | "
                  f"grip={grip:.2f}")

        env.step(action_7d_world)

    print("-" * 60)
    print(f"Complete. {max_steps} steps.")

    if args.save_video and frames:
        os.makedirs(args.output_dir, exist_ok=True)
        import imageio
        video_path = os.path.join(args.output_dir, "ur5e_rollout.mp4")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scene_xml", default="ur5e_scene.xml")
